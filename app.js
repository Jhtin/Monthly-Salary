...
function loadState(){
  ...
  try{
    ...
    if(!Array.isArray(next.employees)||!next.employees.length){
      ...
      next.attendance={};
      const legacy=raw.records||{};
      Object.entries(legacy).forEach(([month,days])=>{
    ...
    next.employees=next.employees.map((e,i)=>({
      id:e.id||`emp-${Date.now()}-${i}`,
      name:e.name||`Employee ${i+1}`,
      rate:Number(e.rate??500),
      color:e.color||EMPLOYEE_COLORS[i%EMPLOYEE_COLORS.length]
    }));
    return next;
  }catch{
...
function render(){
  ...
  $("#dailyRate").textContent=money(emp.rate);
  $("#profileRate").textContent=money(emp.rate);
  $("#daysWorked").textContent=days.length;
  $("#daysCaption").textContent=days.length===1?"day this month":"days this month";
  $("#salaryEarned").textContent=money(days.length*emp.rate);
  $("#salaryFormula").textContent=`${days.length} ${days.length===1?"day":"days"} × ${money(emp.rate)}`;
  ...
  $("#progressBar").style.background=emp.color;
}

function renderEmployees(){
  const wrap=$("#employeeChips");
  if(!wrap)return;
  wrap.innerHTML="";
  state.employees.forEach(emp=>{
    const btn=document.createElement("button");
    btn.type="button";
    btn.className="employee-chip"+(emp.id===state.activeEmployeeId?" active":"");
    btn.style.setProperty("--employee-color",emp.color);
    ...
    btn.onclick=()=>{state.activeEmployeeId=emp.id;save();render()};
...
function renderCalendar(){
  ...
  const y=view.getFullYear(),m=view.getMonth(),monthKey=keyFor(view),monthAttendance=state.attendance[monthKey]||{};
  ...
  for(let d=1;d<=total;d++){
    ...
    if(ids.includes(state.activeEmployeeId))btn.classList.add("worked");
    if(d===today.getDate()&&m===today.getMonth()&&y===today.getFullYear())btn.classList.add("today");
    const names=ids.map(id=>state.employees.find(e=>e.id===id)?.name).filter(Boolean);
    btn.setAttribute("aria-label",`${monthName(view)} ${d}${names.length?`, worked: ${names.join(", ")}`:""}`);
    const number=document.createElement("span");number.className="day-number";number.textContent=d;btn.appendChild(number);
    if(ids.length){
      const badges=document.createElement("span");badges.className="day-employees";
      ids.forEach(id=>{
        const emp=state.employees.find(e=>e.id===id);if(!emp)return;
        const badge=document.createElement("span");badge.className="day-employee-badge";badge.style.background=emp.color;badge.title=emp.name;badge.textContent=initials(emp.name);badges.appendChild(badge);
      });
...
function toggleDay(day){
  const emp=activeEmployee(),key=keyFor(view),dayKey=String(day);
  state.attendance[key]=state.attendance[key]||{};
  const ids=Array.isArray(state.attendance[key][dayKey])?[...state.attendance[key][dayKey]]:[];
  const i=ids.indexOf(emp.id);
  if(i>=0)ids.splice(i,1);else ids.push(emp.id);
  if(ids.length)state.attendance[key][dayKey]=ids;else delete state.attendance[key][dayKey];
  save();render();toast(i>=0?`${emp.name} removed from ${day}`:`${emp.name} marked on ${day}`);
}
...
function renderHistory(){
  ...
  entries.forEach(([key,days])=>{
    const [y,m]=key.split("-").map(Number),d=new Date(y,m-1,1),item=document.createElement("div");item.className="history-item";
    item.innerHTML=`<strong>${monthName(d)}</strong><span>${days.length} workday${days.length===1?"":"s"}</span><div class="money">${money(days.length*emp.rate)}</div>`;
    list.appendChild(item);
  });
}

function faviconFor(url){
  try{return `https://www.google.com/s2/favicons?sz=64&domain_url=${encodeURIComponent(new URL(url).origin)}`}catch{return ""}
}
function createLogo(link,sizeClass=""){
  const box=document.createElement("span");box.className=`link-logo ${sizeClass}`.trim();
  const placeholder=document.createElement("span");placeholder.className="link-logo-placeholder";placeholder.textContent=initials(link.name).slice(0,1);box.appendChild(placeholder);
  const src=(link.logo||"").trim()||faviconFor(link.url);
  if(src){
    const img=document.createElement("img");img.alt="";img.loading="lazy";img.src=src;
...
function renderLinks(){
  const quick=$("#quickLinks"),list=$("#savedLinksList");if(!quick||!list)return;
  quick.innerHTML="";list.innerHTML="";
  const links=Array.isArray(state.links)?state.links:[];
  if(!links.length){quick.innerHTML='<div class="empty-links">No shortcuts yet. Add your first link.</div>';list.innerHTML='<div class="empty-links">No saved links yet.</div>';return}
  links.forEach((link,index)=>{
    const a=document.createElement("a");a.className="quick-link";a.href=link.url;a.target="_blank";a.rel="noopener noreferrer";
    a.appendChild(createLogo(link));
    const label=document.createElement("span");label.className="quick-link-label";label.textContent=link.name;a.appendChild(label);quick.appendChild(a);

    const item=document.createElement("div");item.className="saved-link-item";
    const left=document.createElement("div");left.className="saved-link-with-logo";left.appendChild(createLogo(link,"small-logo"));
    const main=document.createElement("div");main.className="saved-link-main";main.innerHTML=`<strong>${escapeHtml(link.name)}</strong><span>${escapeHtml(link.url)}</span>`;left.appendChild(main);
    const actions=document.createElement("div");actions.className="saved-link-actions";actions.innerHTML=`<a class="icon-button small" href="${escapeAttr(link.url)}" target="_blank" rel="noopener noreferrer" aria-label="Open ${escapeAttr(link.name)}">↗</a><button class="icon-button small danger-link" data-index="${index}" aria-label="Delete ${escapeAttr(link.name)}">×</button>`;
    item.append(left,actions);list.appendChild(item);
  });
...
function addLink(name,url,logo){state.links.push({name:name.trim(),url:normalizeUrl(url),logo:(logo||"").trim()});save();renderLinks();toast("Link saved")}
function deleteLink(index){state.links.splice(index,1);save();renderLinks();toast("Link removed")}
...
$("#confirmClearBtn").onclick=e=>{e.preventDefault();const emp=activeEmployee(),key=keyFor(view),month=state.attendance[key]||{};Object.keys(month).forEach(day=>{month[day]=(month[day]||[]).filter(id=>id!==emp.id);if(!month[day].length)delete month[day]});save();$("#confirmDialog").close();render();toast(`${emp.name}'s month cleared`)};
...
$("#linkForm").onsubmit=e=>{e.preventDefault();const name=$("#linkNameInput").value.trim(),url=$("#linkUrlInput").value.trim(),logo=$("#linkLogoInput").value.trim();if(!name||!url)return;addLink(name,url,logo);e.currentTarget.reset();$("#linkNameInput").focus()};
